...
リポジトリを取得しなくても、GitHub Pages で公開している下記のURLに直接アクセスするだけで利用できます。

- https://yokayoka.github.io/NotoLocshare/index.html
...
### 5m色別レイヤーの配色改善（maff_elvchange3.html）
- 実タイルデータ（能登北部を約42タイル、約25万ピクセル）を解析したところ、標高変化量が**±1m以内に全ピクセルの約99%が集中**していることが判明。
- ±1m以内の帯を、実データの分位点（ヒストグラム平坦化）に基づき細分化（負側9段階・正側10段階、境界は0.01〜0.3m刻みの不均等区分）。±1m〜70m/75mの範囲は従来通り5m/1m刻みの区分を維持し、地すべり・崩壊相当の大きな変化の視認性はそのまま。
  - 全範囲を単純にヒストグラム平坦化すると色階調のほとんどが±0.5m以内に消費され、1〜10m規模の変化がほぼ1色に潰れてしまうため、±1mのみを対象とするハイブリッド方式を採用。
- 配色は「青→白→赤」から**「青→緑→黄→橙→赤」の5色グラデーション**に変更し、0mを黄色に設定。負側は 青→緑→黄、正側は 黄→橙→赤 と多くの色相を経由することで、0付近の細かい区分同士も見分けやすくした（白や単色の中間色に収束して似た色になる問題を回避）。
- 凡例（`legendLabels`）も新しい区分・配色に合わせて更新。

### 雨雲レーダー機能（maff_elvchange3.html / noto2024.html / noto2024_v.html）
- 気象庁の高解像度降水ナウキャストをオーバーレイ「雨雲レーダー（気象庁）」として追加。
- 実況データ（`targetTimes_N1.json`）と予報データ（`targetTimes_N2.json`）を取得し、最新実況から約60分先まで5分刻みで切り替えられる時刻スライダーを表示。
  - 何も操作しなければ自動的に最新の実況を追尾し、5分ごとに更新。
  - スライダーを操作すると、その時刻（実況または予報）を維持したまま表示。
- 表示中は気象庁の降水強度カラースケール（1〜80mm/h以上、7段階）の凡例を表示。凡例・時刻スライダーともレイヤーのON/OFFに連動して表示・非表示が切り替わる。
...
- ベースマップ: 地震前CS立体図 / 地震後CS立体図 / 地理院地図 / 5m色別（青→緑→黄→橙→赤、デフォルト表示。詳細は「5m色別レイヤーの配色改善」の項を参照）
- オーバーレイ: 地震後写真 / 豪雨後写真東・西 / 地震崩壊地東部・西部（GeoJSON） / 旧地すべり地形 / 林野庁判読崩壊地・亀裂（PBFベクトルタイル） / 盛土 / 雨雲レーダー（気象庁）
